...
@section('content')
<div class="container">
    <h1 class="text-gray-900 dark:text-gray-100 text-2xl font-bold mb-4">Edit Lowongan</h1>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        <form action="{{ route('job-postings.update', $jobPosting) }}" method="POST">
            ...
            <div class="mb-4">
                <label for="salary_range" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Gaji</label>
                <input type="text" id="salary_range" name="salary_range" value="{{ old('salary_range', $jobPosting->salary_range) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="skills" name="skills[]" multiple class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    @foreach($skills as $skill)
                        <option value="{{ $skill->id }}" {{ in_array($skill->id, old('skills', $jobPosting->skills->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $skill->skill_name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="date" id="closing_date" name="closing_date" value="{{ old('closing_date', optional($jobPosting->closing_date)->format('Y-m-d')) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            </div>
