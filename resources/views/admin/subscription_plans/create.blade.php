<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Paket Langganan Baru') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.subscription_plans.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="plan_name" id="plan_name" value="{{ old('plan_name') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Contoh: Premium Plan" required>
                        </div>

                        {{-- Price --}}
                        <div class="mb-4">
                            <label for="price" class="block text-gray-700 text-sm font-bold mb-2">Harga (Rp):</label>
                            <input type="number" name="price" id="price" value="{{ old('price') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="150000" min="0" required>
                        </div>

                        {{-- Duration --}}
                        <div class="mb-4">
                            <label for="duration_days" class="block text-gray-700 text-sm font-bold mb-2">Durasi (Hari):</label>
                            <input type="number" name="duration_days" id="duration_days" value="{{ old('duration_days') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="30" min="1" required>
                        </div>

                        {{-- Job Post Limit (Wajib Ada) --}}
                        <div class="mb-4">
                            <label for="job_post_limit" class="block text-gray-700 text-sm font-bold mb-2">Batas Posting Lowongan:</label>
                            <input type="number" name="job_post_limit" id="job_post_limit" value="{{ old('job_post_limit') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Contoh: 10" min="1" required>
                        </div>

                        {{-- Allow Verified Badge (Wajib Ada) --}}
                        <div class="mb-4">
                            ...
                            <select name="allow_verified_badge" id="allow_verified_badge" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                <option value="0" {{ old('allow_verified_badge') == '0' ? 'selected' : '' }}>Tidak</option>
                        ...
                        <div class="flex items-center justify-between mt-6">
                            <a href="{{ route('admin.subscription_plans.index') }}" class="text-gray-600 hover:underline">Kembali</a>
