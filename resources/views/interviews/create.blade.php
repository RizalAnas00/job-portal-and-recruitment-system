...
@section('content')
<div class="container">
    <h1 class="text-gray-900 dark:text-gray-100 text-2xl font-bold mb-4">Jadwalkan Interview</h1>
    ...
    <!-- Info Pelamar -->
    <div class="bg-primary-50 dark:bg-primary-900 border-l-4 border-primary-500 p-4 mb-6">
        <div class="flex">
            ...
            <div class="ml-3">
                <h3 class="text-sm font-medium text-primary-800 dark:text-primary-200">Informasi Pelamar</h3>
                <div class="mt-2 text-sm text-primary-700 dark:text-primary-300">
                    <p><strong>Nama:</strong> {{ $application->jobSeeker->user->name ?? '-' }}</p>
                    <p><strong>Email:</strong> {{ $application->jobSeeker->user->email ?? '-' }}</p>
                    <p><strong>Posisi:</strong> {{ $application->jobPosting->job_title ?? '-' }}</p>
                    <p><strong>Status Lamaran:</strong> <span class="font-semibold">{{ ucfirst($application->status) }}</span></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        <form action="{{ route('interviews.store') }}" method="POST">
            @csrf
            <input type="hidden" name="id_application" value="{{ $application->id }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="interviewer_name" name="interviewer_name" value="{{ old('interviewer_name') }}" 
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="mb-4">
                    ...
                    <input type="datetime-local" id="interview_date" name="interview_date" value="{{ old('interview_date') }}" 
                ...
                <div class="mb-4">
                    ...
                    <select id="interview_type" name="interview_type" 
                        ...
                        <option value="online" {{ old('interview_type') == 'online' ? 'selected' : '' }}>Online</option>
                        <option value="offline" {{ old('interview_type') == 'offline' ? 'selected' : '' }}>Offline</option>
                        <option value="phone_screen" {{ old('interview_type') == 'phone_screen' ? 'selected' : '' }}>Phone Screen</option>
                    </select>
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea id="notes" name="notes" rows="4" 
                    ...
                    placeholder="Informasi tambahan untuk kandidat (persiapan, dokumen yang perlu dibawa, dll)">{{ old('notes') }}</textarea>
            </div>
